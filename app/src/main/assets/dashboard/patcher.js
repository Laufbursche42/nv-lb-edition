...
const IMAGES = {
  ...
  // Meter 3.0.1.6, NT5 Ultra X (9501). 148480 bytes. Cruise (0x52) and zero-start (0x6a) are ungated
  // stock here: the cruise leaf FUN_08014528 stores+forwards the flag with no region compare, and the
  // zero-start leaf FUN_08014a14 only upper-clamps the level to 5 (no region floor), so both work in
  // every region without a patch. Top speed is capped controller-side (no NT5 Ultra X BLDC image is
  // published), so only the warning-tone silences apply here.
  meterUltraX: {

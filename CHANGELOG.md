...
- **More models fully covered after a byte-level re-audit.** Reading each firmware line by line found switchable speed where earlier passes had given up: NT5 Ultra X caps its top speed in the meter (inlined region gate) and is now patched there, so it joins the fully-covered list; the V-series controllers V40i / V40i Pro, V40i Pro II and V3 Pro take the same switchable capZ speed latch as the other V models; the N65i II (10701) controller takes the full latch and K100 Max gets a permanent controller speed unlock (its own MM32 CRC32 seal); and the G5 line plus S40 / S60 gained their warning-beep silences. That lifts the fully-covered count from 12 to 16 models. Every one of these was byte-verified against the stock image and re-seals deterministically; where a switchable patch has no safe home (N65i II 6001 has no free code space, E45 / E60 Pro not yet confirmed) the tool honestly leaves speed stock.
...
- **Ride-log exports are now LEAT-compatible.** The recorded telemetry carries the canonical field names the LEAT desktop tool reads, with units converted (`realSpeed`, `SOC`, `VolPack` in volts, `current` in amps with discharge positive, `power` in kilowatts, `singleMile`, `avgSpeed`, `maxSpeed`). The JSON export is now a plain array of samples, the CSV drops the UTF-8 BOM and the `tsISO` column so the `ts` time axis is read correctly, and the GPX `<speed>` is written in m/s per the GPX convention.
